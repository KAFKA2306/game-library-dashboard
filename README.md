...
Game Library Dashboardは、公式情報で照合したゲーム所蔵を、版・プラットフォーム・証拠種別を分けたまま管理・表示する静的ダッシュボードです。その問題を示した後で、canonical snapshot、sync queue、Work / Edition / PlatformRelease / Holding、white-label catalogなどの技術的な構造を使います。

READMEの入口は [`KAFKA2306/articles#34`](https://github.com/KAFKA2306/articles/issues/34) の「広い問題 → 具体例 → 技術」の編集原則を維持します。Steam画面の観測件数、プレイ履歴、デモ、ソフトウェア、推薦候補、正準所蔵を混同しません。

**公開サイト:** https://kafka2306.github.io/game-library-dashboard/

最新件数は手書きせず `data/game-library.json` を正とします。2026-08-09に確認したSteam画面では `すべてのゲーム (141)` が表示されているため、この表示値をそのまま正準件数へ転記せず、差分監査キューで段階的に突合します。
